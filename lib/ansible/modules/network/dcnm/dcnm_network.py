...
class DcnmNetwork:
        ...
        for want_c in self.want_create:
            ...
            if not found:
                ...
                if not net_id:
                    ...
                    result['response'] = dcnm_send(self.module, method, create_path,
                                                    json.dumps(want_c))
                    fail, result['changed'] = self.handle_response(result['response'], "create")
                    if fail:
                        self.failure(result['response'])
    ...
    def get_diff_query(self):
        ...
        for want_c in self.want_create:
            found_c = (next((net for net in self.have_create if net['networkName'] == want_c['networkName']), None)).copy()
...
def main():
    ...
    if module.params['state'] == 'query':
        ...
        result['response'] = dcnm_net.query

    warnings.append(warn_msg) if warn_msg else []

    if module.check_mode:
        check_mode_results = []

        if dcnm_net.diff_create:
            check_mode_results.append(dcnm_net.diff_create)
        if dcnm_net.diff_attach:
            check_mode_results.append(dcnm_net.diff_attach)
        if dcnm_net.diff_detach:
            check_mode_results.append(dcnm_net.diff_detach)
        if dcnm_net.diff_deploy:
            check_mode_results.append(dcnm_net.diff_deploy)
        if dcnm_net.diff_undeploy:
            check_mode_results.append(dcnm_net.diff_undeploy)
        if dcnm_net.diff_delete:
            check_mode_results.append(dcnm_net.diff_delete)
    ...
    path = '/rest/top-down/fabrics/{}/networks'.format(dcnm_net.fabric)
    bulk_create_path = '/rest/top-down/bulk-create/networks'

    if dcnm_net.diff_create or dcnm_net.diff_attach or dcnm_net.diff_deploy \
            or dcnm_net.diff_delete or dcnm_net.diff_create_update:
        result['changed'] = True
    else:
        module.exit_json(**result)

    method = 'PUT'
    if dcnm_net.diff_create_update:
        for net in dcnm_net.diff_create_update:
            update_path = path + '/{}'.format(net['networkName'])
            result['response'] = dcnm_send(dcnm_net.module, method, update_path, json.dumps(net))
            fail, result['changed'] = dcnm_net.handle_response(result['response'], "create")
            if fail:
                dcnm_net.failure(result['response'])
    ...
    if dcnm_net.diff_detach:
        ...
        result['response'] = dcnm_send(dcnm_net.module, method, detach_path, json.dumps(dcnm_net.diff_detach))
        fail, result['changed'] = dcnm_net.handle_response(result['response'], "attach")
        if fail:
            dcnm_net.failure(result['response'])
    ...
    if dcnm_net.diff_undeploy:
        ...
        result['response'] = dcnm_send(dcnm_net.module, method, deploy_path, json.dumps(dcnm_net.diff_undeploy))
        fail, result['changed'] = dcnm_net.handle_response(result['response'], "deploy")
        if fail:
            dcnm_net.failure(result['response'])

    method = 'DELETE'
    del_failure = ''
    if dcnm_net.diff_delete and dcnm_net.wait_for_del_ready():
        for net, state in dcnm_net.diff_delete.items():
            if state == 'OUT-OF-SYNC':
                del_failure += net + ","
                continue
            delete_path = path + "/" + net
            result['response'] = dcnm_send(dcnm_net.module, method, delete_path)
            fail, result['changed'] = dcnm_net.handle_response(result['response'], "delete")
            if fail:
                dcnm_net.failure(result['response'])

    if del_failure:
        result['response'] = 'Deletion of Networkss {} has failed'.format(del_failure[:-1])
        dcnm_net.failure(result['response'])

    method = 'POST'
    if dcnm_net.diff_create:
        result['response'] = dcnm_send(dcnm_net.module, method, bulk_create_path, json.dumps(dcnm_net.diff_create))
        fail, result['changed'] = dcnm_net.handle_response(result['response'], "create")
        if fail:
            dcnm_net.failure(result['response'])
    ...
    if dcnm_net.diff_attach:
        ...
        result['response'] = dcnm_send(dcnm_net.module, method, attach_path, json.dumps(dcnm_net.diff_attach))
        fail, result['changed'] = dcnm_net.handle_response(result['response'], "attach")
        if fail:
            dcnm_net.failure(result['response'])
    ...
    if dcnm_net.diff_deploy:
        deploy_path = path + '/deployments'
        result['response'] = dcnm_send(dcnm_net.module, method, deploy_path, json.dumps(dcnm_net.diff_deploy))
        fail, result['changed'] = dcnm_net.handle_response(result['response'], "deploy")
        if fail:
            dcnm_net.failure(result['response'])
